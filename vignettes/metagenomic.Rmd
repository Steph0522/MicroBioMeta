...
`method = "jaccard"` matches with the one used for the ordinatrion above, so the ordination and the significance test are evaluating the same notion of dissimilarity. 
...
`beta_decay_plot()` tests whether communities that are geographically closer are also more similar in composition:
...
For the heatmap and ANCOMBC2 versions we need a 2-group subset of the 6-level `Polygon`, in this part we will compare Polygon 2 and 5:
