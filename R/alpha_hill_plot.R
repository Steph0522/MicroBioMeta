...
alpha_hill_plot <- function(
  ...
  facet_config <- if (!is.null(facet_by)) {
    ...
    if (facet_orientation == "horizontal") {
      ggplot2::facet_wrap(~ q, ncol = if (!is.null(n_cols)) n_cols else length(unique(results_largo$q)),
                          scales = if (free_y) "free_y" else "fixed")
    } else {
      ggplot2::facet_wrap(~ q, nrow = if (!is.null(n_rows)) n_rows else length(unique(results_largo$q)),
                          scales = if (free_y) "free_y" else "fixed")
    }
  }
